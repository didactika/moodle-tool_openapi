...
// "<plugin type>_<plugin name>". Must match the folder this plugin installs
// into (<moodle>/admin/tool/openapi/) and the folder name moodle-plugin-ci
// checks out in CI.
...
// integer branch codes (405 = 4.5, 503 = 5.3). A single `main` line covers
// the whole range for now -- see the plan's Fase 0 for why this repository
// does not carry the template's MOODLE_XXX_STABLE branches yet: cutting one
// per Moodle version before any plugin code has diverged between them would
// be ten empty, identical branches, not ten supported versions. A
// MOODLE_XXX_STABLE branch gets cut the day a specific Moodle version
// actually needs code the others don't -- the same principle this
// organisation already applies to its npm packages' vMAJOR.x branches.
//
// ci.yml's test matrix, per its own documented semantics, only exercises
// the two endpoints of this range (405 and 503) -- not the versions in
